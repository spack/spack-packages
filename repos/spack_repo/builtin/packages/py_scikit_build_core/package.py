# Copyright Spack Project Developers. See COPYRIGHT file for details.
#
# SPDX-License-Identifier: (Apache-2.0 OR MIT)

from spack_repo.builtin.build_systems.python import PythonPackage

from spack.package import *


class PyScikitBuildCore(PythonPackage):
    """scikit-build-core is a doubly improved build system generator
    for CPython C/C++/Fortran/Cython extensions. It features several
    improvements over the classic scikit-build build system generator."""

    homepage = "https://github.com/scikit-build/scikit-build-core"
    pypi = "scikit_build_core/scikit_build_core-0.2.0.tar.gz"
    git = "https://github.com/scikit-build/scikit-build-core"

    maintainers("wdconinc")

    license("Apache-2.0")

    version("1.0.3", sha256="a4d7a05978ee37975c37743510c8991e2debce7ef83afb0a07c0c576fd4f16e8")
    version(
        "0.12.2",
        sha256="562e0bbc9de1a354c87825ccf732080268d6582a0200f648e8c4a2dcb1e3736d",
        preferred=True,
    )
    version("0.11.6", sha256="5982ccd839735be99cfd3b92a8847c6c196692f476c215da84b79d2ad12f9f1b")
    version("0.11.5", sha256="8f0a1edb86cb087876f3c699d2a2682012efd8867b390ed37355f13949d0628e")
    version("0.11.1", sha256="4e5988df5cd33f0bdb9967b72663ca99f50383c9bc21d8b24fa40c0661ae72b7")
    version("0.10.7", sha256="04cbb59fe795202a7eeede1849112ee9dcbf3469feebd9b8b36aa541336ac4f8")
    version("0.9.5", sha256="2a4cb119cc968fe87ae05582979657cc0e7be45655798446eabbe490e61ce072")
    version("0.8.2", sha256="50ec24b9568c9aa6e27233deeb2978932bc79856212b30575cbfa4049655c436")
    version("0.7.1", sha256="565f33e15f5aa4514248c508ce3ce40fb6f406f8c3983e891561757b1c9f78ab")
    version("0.6.1", sha256="392254a4ca7235c27a4be98cc24cd708f563171961ce37cff66120ebfda20b7a")
    version("0.6.0", sha256="1bea5ed83610b367f3446badd996f2356690548188d6d38e5b93152df311a7ae")
    version("0.2.0", sha256="d2a76d9447a412038dc5e25dd259b03c25278661a0c7c3da766bb971c1a9acd2")

    variant("pyproject", default=False, description="Enable pyproject.toml support")

    depends_on("c", type="build")  # generated
    depends_on("cxx", type="build")  # generated
    depends_on("fortran", type="build")  # generated


    with default_args(type=("build", "run")):
        depends_on("python@:3.14", when="@:0.12")
        depends_on("python@3.8:", when="@0.11:")
        depends_on("python@3.7:")

    # Build system
    depends_on("py-hatchling@1.24:", when="@0.12:", type=("build", "test"))
    depends_on("py-hatchling", type="build")
    depends_on("py-hatch-vcs@0.4:", when="@0.12:", type=("build", "test"))
    depends_on("py-hatch-vcs", type="build")

    # Dependencies
    with default_args(type=("build", "run")):
        depends_on("py-exceptiongroup@1:", when="@0.9: ^python@:3.10")
        depends_on("py-exceptiongroup", when="^python@:3.10")
        depends_on("py-importlib-metadata@1:", when="@0.9: ^python@:3.7")
        depends_on("py-importlib-metadata", when="@0.3.0: ^python@:3.7")
        depends_on("py-importlib-resources@1.3:", when="^python@:3.8")
        depends_on("py-packaging@23.2:", when="@0.10:")
        depends_on("py-packaging@21.3:", when="@0.9")
        depends_on("py-packaging@20.9:")
        depends_on("py-pathspec@0.12.0:", when="@0.12.2:")
        depends_on("py-pathspec@0.10.1:", when="@0.9:")
        depends_on("py-tomli@1.2.2:", when="@0.9: ^python@:3.10")
        depends_on("py-tomli@1.1:", when="^python@:3.10")
        depends_on("py-typing-extensions@4:", when="@0.12.2: ^python@:3.10")
        depends_on("py-typing-extensions@3.10:", when="@0.8: ^python@:3.8")
        depends_on("py-typing-extensions@3.10:", when="@:0.7 ^python@:3.7")
        depends_on("cmake@3.15:")

        # Optional dependencies
        depends_on("py-pyproject-metadata@0.5:", when="@:0.8 +pyproject")
        depends_on("py-pathspec@0.10.1:", when="@:0.8 +pyproject")

    # Test dependencies
    with default_args(type="test"):
        depends_on("py-build@0.8:", when="@0.9:")
        depends_on("py-build +virtualenv", when="@:0.8")
        depends_on("py-cattrs@22.2:")
        depends_on("py-fastjsonschema@2.16.2:", when="@0.12:")
        depends_on("py-filelock@3.8:", when="@0.12:")
        depends_on("py-hatch-fancy-pypi-readme@23.2:", when="@0.12:")
        depends_on("py-importlib-metadata", when="^python@:3.7")
        depends_on("py-pybind11@2.12:", when="@0.9:")
        depends_on("py-pathspec@0.10.1:", when="@:0.8")
        depends_on("py-pybind11@2.12:", when="@0.9:")
        depends_on("py-pybind11")
        depends_on("py-pyproject-metadata@0.5:", when="@:0.8")
        depends_on("py-pytest@7:")
        depends_on("py-pytest-xdist@3.1:", when="@0.11.4:")
        depends_on("py-pytest-subprocess@1.5:")
        depends_on("py-setuptools-scm@5:", when="@0.12:")
        depends_on("py-setuptools@43:", when="@0.9: ^python@:3.8")
        depends_on("py-setuptools@45:", when="@0.9: ^python@3.9")
        depends_on("py-setuptools@49:", when="@0.9: ^python@3.10:3.11")
        depends_on("py-setuptools@66.1:", when="@0.9: ^python@3.12:")
        depends_on("py-setuptools")
        depends_on("py-typing-extensions@3.10:", when="@0.12: ^python@:3.9")
        depends_on("py-validate-pyproject@0.21:", when="@0.12:")
        depends_on("py-virtualenv@20.0.28:", when="@0.9:")
        depends_on("py-virtualenv", when="@0.6:")
        depends_on("py-wheel@0.40:", when="@0.9:")
        depends_on("py-wheel")

    @run_after("install")
    @on_package_attributes(run_tests=True)
    def install_test(self):
        with working_dir("tests"):
            which("pytest", required=True)()
