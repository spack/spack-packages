...
#    # CLI generator
#    # required py-typer-slim but typer-slim and typer have the same sources on pypi
...
#    ## we likely support other versions, but typing is not consistent across rdkit versions, and mypy complain
    depends_on("rdkit@2024_9_5:2024_9+python", type=("build", "run"))
    ## parsing, data access
    depends_on("py-biopython@1.83:", type=("build", "run"))
    ## save/load heterogeneous python structures
    depends_on("py-antipickle@0.2.0", type=("build", "run"))
    ## Python bindings for the TM-align algorithm
    depends_on("py-tmtools@0.0.3:", type=("build", "run"))
    ## mmcif writing, confirmed to work currently latest 1.0
    ...
    ## versions <0.2.25 do not easily support runtime typechecking
    depends_on("py-jaxtyping@0.2.25:", type=("build", "run"))
    ## compatible typechecker to use with jaxtyping
