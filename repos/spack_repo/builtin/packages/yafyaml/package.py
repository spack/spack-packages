...
class Yafyaml(CMakePackage):
    ...
    conflicts(
        "%oneapi@2025.2", when="@:1.4", msg="oneAPI 2025.2 only works with yafyaml 1.5.1 onwards"
    )
    ...
    def patch(self):
        filter_file(
            ...
            "src/Nodes/BaseNode_implementation.F90",
            string=True,
        )
        filter_file("__RC__)", "rc=status); __VERIFY__(status)", "src/Lexer.F90", string=True)
