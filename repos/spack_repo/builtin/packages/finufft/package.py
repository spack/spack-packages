...
class Finufft(CMakePackage, CudaPackage):
    ...
    # Backport of https://github.com/flatironinstitute/finufft/commit/146e1e0,
    # without which an installed consumer cannot compile.
    patch("install-defines-header.patch", when="@2.5.1")
    # Upstream instead resolves OpenMP from finufftConfig.cmake, which still
    # leaves a C-only consumer unable to use the installed package.
    patch("no-openmp-interface-link.patch", when="@2.5.1 +openmp")
    ...
    def cmake_args(self):
        ...
        args = [
            ...
            # A static libfinufft.a neither contains nor exports its private
            # dependencies, so an installed consumer cannot link it.
            self.define("FINUFFT_STATIC_LINKING", False),
