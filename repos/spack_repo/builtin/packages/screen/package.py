...
class Screen(AutotoolsPackage, GNUMirrorPackage):
    ...
    gnu_mirror_path = "screen/screen-5.0.1.tar.gz"

    license("GPL-3.0-or-later")

    version("5.0.1", sha256="2dae36f4db379ffcd14b691596ba6ec18ac3a9e22bc47ac239789ab58409869d")

    # Older versions deprecated because of various CVEs
    with default_args(deprecated=True):
        ...
        version(
            "3.9.15", sha256="11ea131c224fa759feee3bc6ee2e3d6915a97d2d6da46db968dc24b41de054db"
        )
        version(
            "3.9.11", sha256="f0d6d2eae538688766381c1658e3d4a64c8b4afb3682c2bb33ce96edc840a261"
        )
        version(
            "3.9.10", sha256="3e8df4e1888e59267c37d2a24fa8365cd4d2081392f719579a347a2c6d1386a8"
        )
    ...
    depends_on("autoconf@2.71:", type="build", when="@5.0.1:")
    depends_on("autoconf@2.60:", type="build", when="@4.9.0:4.9.1")
    depends_on("automake", type="build", when="@4.9.0:")
    depends_on("libtool", type="build", when="@4.9.0:")
    depends_on("ncurses")
    depends_on("libxcrypt", when="@3.9:")
    depends_on("linux-pam", when="@5.0.1:")
