...
class NimrodAai(CMakePackage):
    ...
    url = "https://gitlab.com/NIMRODteam/open/nimrod-abstract/-/archive/26.4/nimrod-abstract-26.4.tar.gz"
    ...
    version("26.4", sha256="23f2022e42bb4f249bd05dd52bb7fd5dec8f80acd63ffe83662a56ee8209df5c")
    version("25.7", sha256="d00562ec0942c69fc4e738b11dd537f1ca8bf1d81695e0b06fd4b62b4866fe57")
    ...
    variant("cudss", default=False, description="Whether to link with the cuDSS Direct Solver")
    ...
    variant(
        "ginkgo",
        default=False,
        description="Whether to link with the Ginkgo Linear Algebra Library",
    )
    variant("lapack", default=True, description="Whether to link with LAPACK")
    ...
    variant(
        "superlu", default=True, description="Whether to link SuperLU (DIST with +mpi) library"
    )
    ...
    with when("+ginkgo"):
        ...
        depends_on("ginkgo+cuda", when="+openacc")
    depends_on("lapack", when="+lapack")
    depends_on("mpi", when="+mpi")
    with when("+superlu"):
        depends_on("superlu@7:", when="~mpi")
        with when("+mpi"):
            depends_on("superlu-dist+parmetis")
            depends_on("superlu-dist+cuda", when="+openacc")
    with when("+mpi"):
        conflicts("+superlu", when="~lapack")

    def cmake_args(self):
        args = [
            self.define_from_variant("ENABLE_CUDSS", "cudss"),
            self.define_from_variant("DEBUG", "debug"),
            self.define_from_variant("ENABLE_GINKGO", "ginkgo"),
            ...
            self.define_from_variant("ENABLE_SUPERLU", "superlu"),
