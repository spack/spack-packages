...
# Highly-recommended:
#     WITH_LIBSIXEL          : (default: ON) use libsixel to output images in sixel graphics format
#     WITH_TURBOJPEG         : (default: ON) if enabled, uses this for faster jpeg file loading
#     WITH_POPPLER           : (default: ON) high-quality and faster PDF renderer; needs poppler and cairo (*)
#     WITH_RSVG              : (default: ON) high-quality SVG renderer; needs librsvg and cairo (*)
# Optional:
#     WITH_VIDEO_DECODING    : (default: OFF) allow for video decoding, requires ffmpeg-related libraries
#     WITH_OPENSLIDE_SUPPORT : (default: OFF) openslide is an image format used in scientific applications, rarely used
#     WITH_QOI_IMAGE         : (default: ON) allow decoding of Quite Ok Image format QOI, small and simple
#     WITH_STB_IMAGE         : (default: OFF) compile the simpler STB image library directly into the timg binary 
#                              in cases where Graphicsmagick is not suitable for dependency pruning reasons; 
#                              output can be slower and of less quality, it will always only attempted after other image loading fails;
#                              recommeded to be turned off if you can use GraphicsMagick and want to reduce potential security vectors
#     WITH_VIDEO_DEVICE      : (default: OFF) allows accessing connected video devices, e.g. you can watch your webcam input (requires WITH_VIDEO_DECODING)
#
# (*) if not compiled-in, will fallback to GraphicsMagick, but that typically results in lower quality renderings
#
...
class Timg(CMakePackage):
    ...
    version("1.6.2", sha256="a5fb4443f55552d15a8b22b9ca4cb5874eb1a988d3b98fe31d61d19b2c7b9e56")
    version("1.6.1", sha256="08147c41ce4cea61b6c494ad746e743b7c4501cfd247bec5134e8ede773bf2af")
    version("1.6.0", sha256="9e1b99b4eaed82297ad2ebbde02e3781775e3bba6d3e298d7598be5f4e1c49af")
    version("1.5.3", sha256="ddf2fb1fb2376d31957415d278bc34ff0ef574eb69ef96ddcb564c392d2e4c27")
    ...
    # enforce the glib variant on the upstream poppler dependency whenever the poppler variant is activated for timg
