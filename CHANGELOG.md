...
and no guarentee is made that the environments used to build the binary
cache can be reconcretized with v1.1.X . The binary caches generated by this release
are compatible with all versions of Spack that support the v3 URL
build cache layout.

## Spack package API version

This release continues to use the v2.2 Package API. This is the Package API
version used by the Spack v1.0.0 release. We are guaranteeing that any Spack
newer than v1.0.0 is compatible with this release of the packages repo.
